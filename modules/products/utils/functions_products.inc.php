...
//  exit;
    ...
    //Non checked data:
    $result['cat1'] = $value['cat1'];
    $result['cat2'] = $value['cat2'];
    $result['cat3'] = $value['cat3'];
    $result['cat4'] = $value['cat4'];
    ...
    if ($result['city']==='Select city'){
     ...
     if ($result['food']==='Select food'){
            $error['food']="You need to choose a kind of food";
            $valid = false;
        }
    ...
    if ($result != null && $result){
        ...
        if (!$result['date_reception']) {
            if ($result['date_reception'] == "") {
                $error['date_reception'] = "Disponibility date can't be empty";
                $valid = false;
            } else {
                $error['date_reception'] = 'error disponibility format date (dd/mm/yyyy)';
                $valid = false;
            }
        }
